...
export class PaginationPipe implements PipeTransform {
  ...
  transform(value: any, metadata: ArgumentMetadata): PaginationParams {
    ...
    const skip =
      value?.skip !== undefined ? parseInt(value.skip, 10) : this.DEFAULT_SKIP;

    // Parse take — default to 10 if not provided or NaN
    const take =
      value?.take !== undefined ? parseInt(value.take, 10) : this.DEFAULT_TAKE;

    // Validate skip: must be a non-negative integer
    if (isNaN(skip) || skip < 0) {
      throw new BadRequestException(
        'El parámetro "skip" debe ser un entero mayor o igual a 0.',
      );
    }
    ...
    if (isNaN(take) || take < 1 || take > this.MAX_TAKE) {
      throw new BadRequestException(
        `El parámetro "take" debe estar entre 1 y ${this.MAX_TAKE}.`,
      );
    }
